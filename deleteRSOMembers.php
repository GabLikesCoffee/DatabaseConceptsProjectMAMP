...
<body>
    ...
    <form method="post" name="submit">
        ...
        <br />
        <button name="submit" type="submit">Remove Member</button>
    </form>
...
$sqlEvents = "SELECT RM.userId, RM.RSOname FROM RSO R, RSOmembers RM WHERE RM.RSOname = R.name AND R.creator = '$userId' AND RM.userId <> '$userId'";
...
if ($numExists > 0) {
    ...
    echo " 
        <script type=\"text/javascript\">
            ...
            function removeFill(id, RSOname){
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "
            <script type=\"text/javascript\">
                ...
                insertTable += '<td><button onclick=\"removeFill(\'$row[userId]\', \'$row[RSOname]\')\">remove</button></td>';
...
if (isset($_POST['submit']) && !empty($_POST['memberId'])) {
    ...
    $sql = "DELETE FROM `RSOmembers` WHERE `userId` = '$memberId' AND `RSOname` = '$RSOname'";
    if ($conn->query($sql) === TRUE) {
        echo "Member Removed from RSO!";
    } else {
